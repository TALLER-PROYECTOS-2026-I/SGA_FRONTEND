# taller_frontend

# Adding CODEOWNERS file

Pruebas DevSecOps - Commit 1.

1er cambio: Se agregó la función del Sonar al security-dev.yml

2do cambio: Se añadió como Secret el SONAR_TOKEN del repo.

3er cambio: Se agrega el archivo sonar-project.properties para visualizarlo en en el SonarQube. (Verificar)

4to cambio: Se modificó la Key del sonar-project.properties. (Nuevos cambios)

5to cambio: Implementación del SonarQube. (Cambio de nombres)

6to cambio: Se realizaron cambios en el properties security-dev.yml (args) Nuevo cambio. (New change_1)
