...
export function EvaluationSection({
  ...
  if (evaluacion?.formulaEvaluacion) {
    return (
      <>
        ...
        {evaluacion.formulaEvaluacion.variables?.map((variable) => (
          <FormulaComponentLine
            key={`var-${variable.codigo ?? variable.descripcion}`}
            item={variable}
            styles={styles}
          />
        ))}

        {evaluacion.formulaEvaluacion.subformulas?.map((subformula) => (
          <Text
            key={`subformula-${subformula.variableCodigo ?? subformula.expresion}`}
  ...
  return (
    <>
      ...
      {evaluacion?.componentesPF?.map((item) => (
        <FormulaComponentLine
          key={`pf-${item.codigo ?? item.descripcion}`}
          item={item}
          styles={styles}
        />
      ))}
      ...
      {evaluacion?.componentesPE?.map((item) => (
        <FormulaComponentLine
          key={`pe-${item.codigo ?? item.descripcion}`}
          item={item}
          styles={styles}
        />
      ))}
