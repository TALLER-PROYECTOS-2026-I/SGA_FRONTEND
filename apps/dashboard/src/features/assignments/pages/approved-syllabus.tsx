...
export default function ApprovedSyllabus() {
  ...
  }, [syllabusIdParam, courseCodeParam, courseNameParam, statusParam]);
  ...
  const fetchCompleteSyllabusData = async (): Promise<CompleteSyllabus> => {
    if (!selectedSyllabus) {
  ...
  const loadCompleteSyllabus = async () => {
    const data = await fetchCompleteSyllabusData();
